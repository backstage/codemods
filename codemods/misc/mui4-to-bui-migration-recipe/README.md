...
| 10  | [`@backstage/migrate-mui-link-to-bui-link`](https://go.codemod.com/registry?q=backstage+migrate-mui-link-to-bui-link)                       | Links                     | Migrates MUI `Link` to BUI `Link` (skips `@backstage/core-components`)                  |
...
`@backstage/migrate-mui-layout-to-bui-layout` never maps Paper to `Surface`. Bare Paper → `Box bg="neutral"`; card-like Paper → `Card`; elevation / ambiguous Paper → left with `TODO(backstage-codemod)`.
