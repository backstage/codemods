...
const transform: Codemod<CSS> = async (root) => {
  ...
  for (const className of classNames) {
    ...
    const classSelector = className
      .ancestors()
      .find((a) => a.is("class_selector"));

    if (!classSelector) continue;

    const parentSelector = classSelector
      .ancestors()
      .find(
        (a) =>
          a.is("child_selector") || a.is("descendant_selector"),
      );
    ...
    if (parentSelector) {
      ...
      const isAncestorSide =
        firstChild && classSelector.id() === firstChild.id();

      if (isAncestorSide) {
        // Find the enclosing rule_set to add TODO comment before it
        const ruleSet = parentSelector
          .ancestors()
          .find((a) => a.is("rule_set"));
