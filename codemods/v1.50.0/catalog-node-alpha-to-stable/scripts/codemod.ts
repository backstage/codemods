...
function recordMigration(outcome: Outcome, exportName: string, reason: Reason): void {
...
function extractSpecifiers(node: SgNode<TSX>, specifierKind: 'import_specifier' | 'export_specifier'): SpecifierInfo[] {
...
function isTypeOnlyStatement(node: SgNode<TSX, 'import_statement' | 'export_statement'>): boolean {
...
function collectBodyReferences(spec: SpecifierInfo, rootFilename: string): SgNode<TSX>[] {
  ...
  for (const refGroup of spec.localNameNode.references()) {
     ...
     */
    if (refGroup.root.filename() !== rootFilename) {
      continue
    }
    for (const refNode of refGroup.nodes) {
      if (refNode.id() === declarationId) {
        continue
      }
      refs.push(refNode)
    }
...
function mergeIntoTypeImport(existingImport: SgNode<TSX>, newSpecTexts: string[]): Edit {
  ...
  const existingSpecs = namedImports.findAll({ rule: { kind: 'import_specifier' } }).map((s) => s.text())

  const allSpecs = [...existingSpecs, ...newSpecTexts]
  const source = existingImport.find({ rule: { kind: 'string_fragment' } })?.text()
...
const transform: Codemod<TSX> = async (root) => {
  ...
  const alphaImportStatements = findStatementsFrom(rootNode, 'import_statement', ALPHA_SOURCE)
...
function processImports(
  ...
  const stableImportStatements = findStatementsFrom(rootNode, 'import_statement', STABLE_SOURCE)
  const firstStableTypeImport = stableImportStatements.find((s) => isTypeOnlyStatement(s))
  const firstStableValueImport = stableImportStatements.find((s) => !isTypeOnlyStatement(s))

  const backendImportStatements = findStatementsFrom(rootNode, 'import_statement', BACKEND_API_SOURCE)
  const [firstBackendImport] = backendImportStatements
  ...
  for (const alphaImport of alphaImportStatements) {
    ...
    if (stabilized.length === 0 && superseded.length === 0) {
      continue
    }
    ...
    if (stabilized.length > 0) {
      ...
      if (existingStableValue) {
        ...
        alphaReplacement = alphaReplacement ? `${newImport}\n${alphaReplacement}` : newImport
      }
    ...
    if (superseded.length > 0) {
      const hasValueExport = superseded.some((s) => s.importedName === SUPERSEDED_VALUE_EXPORT)
      ...
      if (hasValueExport) {
        if (firstBackendImport) {
          ...
          const newImport = buildNamedStatement('import', ['coreServices'], BACKEND_API_SOURCE, false)
          alphaReplacement = alphaReplacement ? `${alphaReplacement}\n${newImport}` : newImport
        }
      }

      for (const s of superseded) {
        if (s.importedName === SUPERSEDED_VALUE_EXPORT) {
          recordMigration('auto-migrated', s.importedName, 'superseded-core-service-rewrite')
        }
      }
    }
...
function processExports(rootNode: SgNode<TSX, 'program'>, edits: Edit[]): void {
  const alphaExports = findStatementsFrom(rootNode, 'export_statement', ALPHA_SOURCE)
  ...
  for (const alphaExport of alphaExports) {
    ...
    if (stabilized.length > 0) {
      ...
      replacement = replacement ? `${stableExport}\n${replacement}` : stableExport
...
function warnNamespaceImports(alphaImportStatements: SgNode<TSX, 'import_statement'>[]): void {
...
function warnDanglingSupersededTypes(superseded: SpecifierInfo[], filename: string): void {
  const typeExports = superseded.filter((s) => s.importedName !== SUPERSEDED_VALUE_EXPORT)
  ...
  for (const spec of typeExports) {
    ...
    if (bodyRefs.length > 0) {
      ...
      recordMigration('manual-required', spec.importedName, 'dangling-type-reference')
    } else {
      recordMigration('auto-migrated', spec.importedName, 'superseded-type-removed')
    }
  }
}

function replaceSupersededReferences(edits: Edit[], specs: SpecifierInfo[], filename: string): void {
