...
function recordMigration(outcome: Outcome, reason: Reason, interfaceName: string): void {
 ...
 */
const TARGET_INTERFACES: ReadonlyMap<string, { source: string; optionsType: string; optionsRequired: boolean }> =
  new Map([
    [
      'CatalogApi',
      {
...
function objectHasUpdateLocation(objectNode: SgNode<TSX>): boolean {
  return hasUpdateLocationProperty(objectNode) || hasUpdateLocationShorthand(objectNode)
}
 ...
 */
function insertPropertyIntoObject(objectNode: SgNode<TSX>, stubValue: string): Edit | null {
  const pairs = objectNode.findAll({ rule: { kind: 'pair' } })
  const shorthands = objectNode.findAll({
    rule: { kind: 'shorthand_property_identifier' },
  })
  const lastProperty = [...pairs, ...shorthands].sort((a, b) => a.range().end.index - b.range().end.index)
  const lastProp = lastProperty.at(-1)
  ...
  if (lastProp) {
    ...
    const hasTrailingComma = nextSibling?.text() === ','
    ...
    return {
      startPos: hasTrailingComma ? nextSibling.range().end.index : lastPropEnd,
      endPos: hasTrailingComma ? nextSibling.range().end.index : lastPropEnd,
...
function processObjectLiteral(
  ...
  if (processedObjectIds.has(obj.id())) {
    return
  }
 ...
 */
function findCatalogTypeInGeneric(typeAnnotation: SgNode<TSX>): string | null {
 ...
 */
function resolveInterfaceAlias(rootNode: SgNode<TSX, 'program'>, interfaceName: string): string | null {
 ...
 */
function verifyTypeDefinition(typeIdNode: SgNode<TSX>, expectedSource: string): boolean {
 ...
 */
function isInsideKind(node: SgNode<TSX>, ancestorKind: string, boundaryKind: string): boolean {
  let current: SgNode<TSX> | null = node.parent()
  while (current) {
    const kind = current.kind()
    if (kind === ancestorKind) {
      return true
    }
 ...
 */
function isCallArgument(node: SgNode<TSX>, boundaryKind: string): boolean {
  let current: SgNode<TSX> | null = node.parent()
  while (current) {
    const kind = current.kind()
...
function getArrowReturnType(arrowFn: SgNode<TSX>): SgNode<TSX> | null {
  ...
  for (const ann of allAnnotations) {
    ...
    if (!isInsideKind(ann, 'formal_parameters', 'arrow_function')) {
      return ann
    }
  }
  return null
...
const transform: Codemod<TSX> = async (root) => {
  ...
  for (const classDecl of classDeclarations) {
    ...
    const stub = buildClassStub(matchedInterface.optionsType, matchedInterface.optionsRequired)
  ...
  for (const declarator of variableDeclarators) {
    ...
    if (!importedInterfaces.has(matchedInterfaceName)) {
      continue
    }
  ...
  for (const declarator of variableDeclarators) {
    ...
    if (isInsideKind(typeAnnotation, 'arrow_function', 'variable_declarator')) {
      continue
    }
    ...
    if (isCallArgument(objectLiteral, 'variable_declarator')) {
      continue
    }
  ...
  for (const funcDecl of functionDeclarations) {
    ...
    if (typeId.inside({ rule: { kind: 'type_arguments' } })) {
      continue
    }
  ...
  if (needsLocationImport) {
    ...
    if (!existingLocationImport) {
      ...
      if (locationEdit) {
        edits.push(locationEdit)
      }
    }

    // Also ensure the options type is imported for CatalogApi classes
    for (const info of importedInterfaces.values()) {
