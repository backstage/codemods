    ...
    rule: {
      any: [
        ...
        // Optional chaining: result?.prop
        {
          ...
          all: [{ has: { kind: 'optional_chain' } }, { has: { kind: 'identifier', regex: `^${escapeRegex(name)}$` } }],
        },
...
const transform: Codemod<TSX> = async (root) => {
  ...
  for (const typeId of dialogApiTypeIds) {
    ...
    if (typeAnnotation?.kind() !== 'type_annotation') {
      continue
    }
  ...
  for (const call of calls) {
    ...
    if (methodName !== 'show' && methodName !== 'showModal') {
      continue
    }
    ...
    if (!dialogApiReceiverNames.has(receiverName)) {
      continue
    }
    ...
    if (methodName === 'show') {
      ...
      const varDeclarator = ancestors.find((a) => a.kind() === 'variable_declarator')
      if (varDeclarator) {
                ...
                if (params) {
                  ...
                  if (params.kind() === 'identifier') {
                    ...
                    const firstParam = params
                      .children()
                      .find((c) => c.kind() === 'identifier' || c.kind() === 'required_parameter')
                    if (firstParam) {
                      if (firstParam.kind() === 'identifier') {
                        thenParamBinding = firstParam
                      } else {
                        // required_parameter — find the identifier child
                        const paramName = firstParam.children().find((c) => c.kind() === 'identifier')
                        if (paramName) {
  ...
  for (const site of callSites) {
    if (site.resultBinding) {
      allBindings.push(site.resultBinding)
    }
    if (site.thenParamBinding) {
      allBindings.push(site.thenParamBinding)
    }
  }
  ...
  for (const bindingId of allBindings) {
    ...
    for (const fileRef of refs) {
      if (fileRef.root.filename() === currentFile) {
        ...
        for (const refNode of fileRef.nodes) {
          ...
          if (crossCommentedIds.has(refStmtId)) {
            continue
          }
    ...
    for (const refNode of refNodes) {
      ...
      if (dismissalCommentedStmtIds.has(refStmtId) || commentedStatementIds.has(refStmtId)) {
        continue
      }
  ...
  for (const site of callSites) {
    ...
    if (renameCommentedStmtIds.has(stmtId)) {
      continue
    }
    renameCommentedStmtIds.add(stmtId)

    // Determine the appropriate TODO comment
    const todoComment = site.methodName === 'show' ? SHOW_TODO : SHOW_MODAL_TODO
  ...
  const result = await Promise.resolve(allEdits.length > 0 ? rootNode.commitEdits(allEdits) : null)
  return result
}
