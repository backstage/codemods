...
function callHasArguments(callNode: SgNode<TSX>): boolean {
  ...
  return args.children().some((c) => c.isNamed() && !c.is('comment'))
}
 ...
 */
function findExportStatementsFrom(rootNode: SgNode<TSX>, source: string): SgNode<TSX, 'export_statement'>[] {
  return rootNode.findAll({
    rule: {
...
function buildNamedStatement(keyword: 'import' | 'export', specTexts: string[], source: string): string {
...
function handleNamespaceImport(rootNode: SgNode<TSX>): Edit[] {
  ...
  for (const importStmt of importStatements) {
    ...
    const externalUsageCount = allAliasUsages.length - importIdentifiers.length - callIdentifiers.length
...
function handleDynamicImport(rootNode: SgNode<TSX>): Edit[] {
  ...
  for (const decl of dynamicImportDecls) {
    ...
    const indent = source.slice(declStartPos, declRange.start.index)
...
const transform: Codemod<TSX> = async (root) => {
  ...
  for (const callStmt of callStatements) {
    ...
    if (entireImportRemoved) {
      while (startPos >= 2 && source[startPos - 1] === '\n' && source[startPos - 2] === '\n') {
        startPos -= 1
      }
    }
