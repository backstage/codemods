 ...
 */
function extractSchemaFromZodCallBody(callNode: SgNode<TSX>, baseIndent: string, contentIndent: string): string | null {
 ...
 */
function extractObjectContentFromText(text: string, baseIndent: string, contentIndent: string): string {
  // Remove outer braces
 ...
 */
function extractObjectContent(objectNode: SgNode<TSX>, baseIndent: string, contentIndent: string): string {
 ...
 */
function reindentContent(inner: string, baseIndent: string, contentIndent: string): string {
 ...
 */
function processCallbackFields(schemaObject: SgNode<TSX>, contentIndent: string): string | null {
...
function rebuildImportWithout(importStmt: SgNode<TSX>, specifiersToRemove: Set<string>): string {
  ...
  const isTypeOnly = importStmt.children().some((c) => c.text() === 'type' && c.kind() !== 'import_clause')
...
const transform: Codemod<TSX> = async (root) => {
  ...
  for (const configPair of configPairs) {
    const configValue = configPair.field('value')
    if (!configValue || !configValue.is('object')) {
      continue
    }
    ...
    if (schemaValue.is('call_expression')) {
      ...
      if (callee) {
        const innerContent = extractSchemaFromZodCallBody(schemaValue, baseIndent, contentIndent)
    ...
    if (schemaValue.is('object')) {
      ...
      if (callbackResult !== null) {
        edits.push(configPair.replace(`configSchema: {\n${contentIndent}${callbackResult},\n${baseIndent}}`))
        needsZodImport = true
        transformedAny = true
        migrationMetric.increment({
          pattern: 'callback',
  ...
  if (hasSchemaImport && transformedAny) {
    ...
    for (const importStmt of importStatements) {
      ...
      if (specifiers.length === 1) {
        ...
        const adjustedEnd = endPos < fullText.length && fullText[endPos] === '\n' ? endPos + 1 : endPos
        edits.push({
          startPos,
          endPos: adjustedEnd,
          insertedText: '',
        ...
        const rebuilt = rebuildImportWithout(importStmt, new Set(['createSchemaFromZod']))
  ...
  if (hasSchemaImport && transformedAny) {
    ...
    for (const refNode of remainingRefs) {
      ...
      if (containingStmt) {
        ...
        if (flaggedStmtPositions.has(stmtStart)) {
          continue
        }
        ...
        edits.push({
          startPos: stmtStart,
          endPos: stmtStart,
          insertedText: '// TODO: createSchemaFromZod was removed - update this type annotation\n',
        })
        migrationMetric.increment({
